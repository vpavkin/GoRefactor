...
func init() {
	...
	for _, s := range ast.BasicTypes {
		PredeclaredTypes[s] = &BasicTypeSymbol{
			&TypeSymbol{Obj: &ast.Object{Kind: ast.Typ, Name: s}, Posits: new(vector.Vector)}}
	}
	...
	for _, s := range builtIn {
		PredeclaredFunctions[s] = &FunctionSymbol{Obj: &ast.Object{Kind: ast.Fun, Name: s}, FunctionType: predeclaredFunctionTypes[s], Posits: new(vector.Vector)}
	}
...
var RegisterPositions bool = true

//for handling getBaseType cycles
var visitedTypes map[string]ITypeSymbol
/* Interfaces */
...
func NewOccurence(pos token.Position) Occurence {
	return Occurence{pos}
}
...
func (ts TypeSymbol) AddPosition(o Occurence) {
	if RegisterPositions {
		ts.Posits.Push(o)
	}
}
func (ts PackageSymbol) AddPosition(o Occurence) {
	if RegisterPositions {
		ts.Posits.Push(o)
	}
}
func (ts VariableSymbol) AddPosition(o Occurence) {
	if RegisterPositions {
		ts.Posits.Push(o)
	}
}
func (ts FunctionSymbol) AddPosition(o Occurence) {
	if RegisterPositions {
		ts.Posits.Push(o)
	}
}
...
func getBaseType(sym ITypeSymbol) (ITypeSymbol, bool) {
	if _, ok := visitedTypes[sym.Name()]; ok {
		return nil, true
	}
	if sym.Name() != "" {
		visitedTypes[sym.Name()] = sym
	}
	switch t := sym.(type) {
...
func (ps *PackageSymbol) SetMethods(*SymbolTable) {panic("mustn't call ITypeSymbol methods on PackageSymbol")}
func (ps *PackageSymbol) Methods() *SymbolTable   {panic("mustn't call ITypeSymbol methods on PackageSymbol"); return nil}
func (ps *PackageSymbol) AddMethod(meth Symbol)   {panic("mustn't call ITypeSymbol methods on PackageSymbol")}
func (ps *PackageSymbol) Copy() ITypeSymbol       {panic("mustn't call ITypeSymbol methods on PackageSymbol"); return nil}
